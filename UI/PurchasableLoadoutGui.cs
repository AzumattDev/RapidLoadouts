...
public class PurchasableLoadoutGui : MonoBehaviour
{
    ...
    public static PurchasableLoadoutGui? instance => m_instance;
    ...
    public static List<ItemSet?> GetAvailableSets()
    {
        ...
        return availableSets.Where(x => x != null && (string.IsNullOrEmpty(x.m_requiredGlobalKey) || ZoneSystem.instance.GetGlobalKey(x.m_requiredGlobalKey))).ToList();
    }
    ...
    public static void FillList()
    {
        ...
        for (int index2 = 0; index2 < availableItems.Count; ++index2)
        {
            ...
            try
            {
                Utils.FindChild(element.transform, "coin icon").GetComponent<Image>().sprite = ObjectDB.instance.GetItemPrefab(itemSet?.m_prefabCost).GetComponent<ItemDrop>().m_itemData.GetIcon();
            }
            catch
            {
